...
def _resumir_evidencia(brain: ProjectBrain) -> dict:
    ...
    return {
        ...
        "headline_do_anuncio": _truncar(brain.origem.source_headline, 200),
        "copy_do_anuncio": _truncar(brain.origem.source_copy, 500),
...
def _tem_evidencia_minima(contexto: dict) -> bool:
    ...
    return any([
        contexto.get("headline_do_anuncio"),
        contexto.get("copy_do_anuncio"),
        contexto.get("nicho"),
...
def _prompt(contexto: dict) -> list[dict]:
    ...
    sistema = (
        ...
        "5. Responda SOMENTE com um JSON valido (sem markdown, sem texto "
...
def _validar_candidato(bruto: dict) -> dict | None:
    ...
    return {
        "product_type": tipo,
        "audience_hypothesis": _str_ou_none(bruto.get("audience_hypothesis")),
        "problem": _str_ou_none(bruto.get("problem")),
        "why_it_fits": _str_ou_none(bruto.get("why_it_fits")),
